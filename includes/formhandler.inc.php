...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $query = "INSERT INTO students (`studentName`, `studentFirst`, `studentLast`, `studentAddress`, `studentPhone`, `studentEmail`, `studentPword`) VALUES (?, ?, ?, ?, ?, ?, ?);";
        /*($studentName, $studentFirst, $studentLast, $studentAddress`, $studentPhone, $studentEmail, $studentPword);";*/

        $stmt = $pdo->prepare($query);

        $stmt->execute([$studentName, $studentFirst, $studentLast, $studentAddress, $studentPhone, $studentEmail, $studentPword]);
